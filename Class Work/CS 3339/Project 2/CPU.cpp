...
void CPU::decode() {
  ...
  // Set safe
  writeDest = false, opIsMultDiv = false, opIsStore = false, opIsLoad = false;
  aluOp = ADD;
  ...
  switch(opcode) {
    case 0x00:
      switch(funct) {
        ...
        case 0x10: D(cout << "mfhi " << regNames[rd]);
                   writeDest = true; destReg = rd;
                   aluOp = ADD; // Pass through the hi value without any changes
                   aluSrc1 = hi;
                   aluSrc2 = regFile[REG_ZERO];
                   break;
        case 0x12: D(cout << "mflo " << regNames[rd]);
                   writeDest = true; destReg = rd;
                   aluOp = ADD; // Pass through the lo value without any changes
                   aluSrc1 = lo; 
                   aluSrc2 = regFile[REG_ZERO];
                   break;
        case 0x18: D(cout << "mult " << regNames[rs] << ", " << regNames[rt]);
                   writeDest = false; // Result is sent to hi and low
                   opIsMultDiv = true;
                   aluOp = MUL;
                   aluSrc1 = regFile[rs];
                   aluSrc2 = regFile[rt];
        ...
        case 0x23: D(cout << "subu " << regNames[rd] << ", " << regNames[rs] << ", " << regNames[rt]);
                   ...
                   aluOp = ADD;
                   aluSrc1 = regFile[rs];
                   aluSrc2 = -1 * regFile[rt]; // subtract by adding a negative 
                   break; 
    ...
    case 0x02: D(cout << "j " << hex << ((pc & 0xf0000000) | addr << 2)); // P1: pc + 4
               writeDest = false;
               aluOp = ADD; // pc should pass though without changes
               aluSrc1 = pc;
               aluSrc2 = regFile[REG_ZERO]; 
    ...
    case 0x04: D(cout << "beq " << regNames[rs] << ", " << regNames[rt] << ", " << pc + (simm << 2));
               if( regFile[rs] == regFile[rt] ) // Add pc to simm value if equal
                pc = pc + (simm << 2);
               break; 
    case 0x05: D(cout << "bne " << regNames[rs] << ", " << regNames[rt] << ", " << pc + (simm << 2));
               if( regFile[rs] != regFile[rt] ) // Add pc to simm value if not equal
                pc = pc + (simm << 2);
               break;  
    ...
    case 0x0f: D(cout << "lui " << regNames[rt] << ", " << dec << simm);
               writeDest = true; destReg = rt;
               aluOp = ADD;
               aluSrc1 = simm << 16; // Pass sim << 16 without any modifications
               aluSrc2 = regFile[REG_ZERO];
               break; 
    ...
    case 0x23: D(cout << "lw " << regNames[rt] << ", " << dec << simm << "(" << regNames[rs] << ")");
               ...
               opIsLoad = true;
               aluOp = ADD;
               aluSrc1 = regFile[REG_ZERO]; // Pass rs + simm without any changes
    ...
    case 0x2b: D(cout << "sw " << regNames[rt] << ", " << dec << simm << "(" << regNames[rs] << ")");
               ...
               aluSrc1 = simm;
               aluSrc2 = regFile[rs];
               storeData = regFile[rt]; // Set store data directly 
               break; 
    default: cerr << "unimplemented instruction: pc = 0x" << hex << pc - 4 << endl;
  }
  D(cout << endl);
